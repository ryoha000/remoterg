...
ExtractResult ExtractCaptureTimeFromFrame(const webrtc::VideoFrame& frame) {
  ...
  for (size_t i = 0; i < infos.size(); ++i) {
    ...
    int64_t capture_local_ntp_ms = capture_ntp_ms;
    ...
    if (local_offset.has_value()) {
      ...
      capture_local_ntp_ms += local_offset_ms;
 ...
 public:
  ...
  void OnFrame(const webrtc::VideoFrame& frame) override {
    ...
    if (success_count <= 5 || frame_no % kFrameLogInterval == 1) {
      LOGD(
          "ACT frame=%llu success=%llu info_index=%zu infos=%zu capture_ntp_ms=%lld offset_applied=%d offset_ms=%lld capture_unix_ms=%lld timestamp_us=%lld",
          static_cast<unsigned long long>(frame_no),
