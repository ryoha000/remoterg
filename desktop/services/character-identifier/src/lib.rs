...
pub use matcher::{IdentifiedCharacter, Matcher};
...
impl CharacterIdentifier {
    ...
    pub fn set_device(&mut self, device: DeviceType) -> Result<()> {
        ...
        self.face_detector =
            FaceDetector::new(self.models_dir.join("yolov8_animeface.onnx"), use_gpu)?;
        ...
        if cache_file.exists() {
            if let Ok(data) = tokio::fs::read(&cache_file).await {
                if let Ok(embeddings) =
                    bincode::deserialize::<Vec<(String, Vec<f32>, Vec<String>)>>(&data[..])
                {
                    ...
                    if let Some(bbox) = boxes.first() {
                        ...
                        if let Ok(emb) = self.embedder.embed(&crop) {
                            let tags = self
                                .tagger
                                .predict(&crop, 0.3)
                                .map(|res| res.into_iter().map(|t| t.tag).collect())
                                .unwrap_or_default();
    ...
    pub fn identify(&mut self, screenshot: &[u8]) -> Result<Vec<IdentifiedCharacter>> {
        ...
        let results = Matcher::match_characters(
            &face_embeddings,
