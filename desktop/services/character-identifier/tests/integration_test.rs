...
fn init_tracing() {
    INIT_TRACING.call_once(|| {
        let filter =
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("debug,ort=warn"));
...
async fn test_character_identification_pipeline() {
    ...
    if characters_dir.exists() {
        for entry in
            fs::read_dir(&characters_dir).expect("charactersディレクトリの読み込みに失敗しました")
        {
            ...
            if path.is_file() {
                if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                    ...
                    if ext == "png" || ext == "jpg" || ext == "jpeg" || ext == "webp" {
                        ...
                        let img_data =
                            fs::read(&path).expect("画像ファイルの読み込みに失敗しました");
                        ...
                        if let Ok(img) = image::load_from_memory(&img_data) {
                            if let Ok(mut boxes) = face_detector.detect(&img) {
                                ...
                                if let Some(bbox) = boxes.first() {
                                    ...
                                    let crop_w =
                                        (bbox.x2 - bbox.x1).min(w as f32 - crop_x as f32) as u32;
                                    let crop_h =
                                        (bbox.y2 - bbox.y1).min(h as f32 - crop_y as f32) as u32;

                                    if crop_w > 0 && crop_h > 0 {
                                        let crop = img.crop_imm(crop_x, crop_y, crop_w, crop_h);
                                        let save_path =
                                            char_cropped_dir.join(format!("{}_face.png", name));
        ...
        println!(
            "警告: {:?} が存在しなかったため作成しました。画像を配置してください。",
            characters_dir
        );
    }
    ...
    assert!(
        !characters.is_empty(),
        "テスト用のキャラクター画像が1枚も見つかりません。{:?} に画像を配置してください",
        characters_dir
    );
    println!(
        "キャラクター画像読み込み完了 ({}人): {:?}",
    ...
    // 特徴量の抽出とキャッシュの保存を実行
    identifier
        .register_references(&characters, &cache_dir, "test_game_id")
        .await
