...
///
/// # Returns
/// NV12バッファ（`width * height + width * height / 2`バイト）
/// レイアウト: Y平面 + UV平面（インターリーブ）
pub fn rgba_to_nv12(rgba: &[u8], width: usize, height: usize, src_width: usize) -> Vec<u8> {
    let y_plane_size = width * height;
    let uv_plane_size = y_plane_size / 2; // UV平面はインターリーブなので width * height / 2
    let total_size = y_plane_size + uv_plane_size;

    let mut y = vec![0u8; y_plane_size];
    let mut uv = vec![0u8; uv_plane_size];

    // libyuvのABGRToNV12を使用
    // ABGRはメモリ上ではRGBAと同じ順序（R, G, B, A）
    unsafe {
        let result = libyuv_sys::ABGRToNV12(
            ...
            width as i32, // dst_stride_y
            uv.as_mut_ptr(),
            width as i32, // dst_stride_uv: NV12ではUV平面のストライドはwidth
            width as i32,
            height as i32,
        );
