...
use crate::matcher::{MatchResult, MatchMethod};
...
pub fn score_and_select_all(matches: Vec<MatchResult>) -> Option<Vec<ScoredCandidate>> {
    ...
    for mut m in matches {
        ...
        // Proximity penalty
        if m.match_method != MatchMethod::Exact && m.match_method != MatchMethod::ExactSpaceAgnostic && m.match_method != MatchMethod::Prefix {
    ...
    let mut all_structured_matches: Vec<MatchResult> = grouped.values().flatten().cloned().collect();
    ...
    for (vndb_id, game_matches) in grouped {
        ...
        let cross_bonus = if has_title_match && has_brand_match { 0.1 } else { 0.0 };
    ...
    let valid_candidates: Vec<ScoredCandidate> = candidates.into_iter().filter(|c| c.final_score >= 0.4).collect();
