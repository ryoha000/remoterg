    ...
    for (i, part) in parts.into_iter().enumerate() {
        ...
        if i == len - 1 {
            ...
            if let Some(name) = path_obj.file_stem() {
                ...
                if ENGINE_NAMES.contains(&name_lower.as_str()) || UTILITY_FILENAMES.contains(&name_lower.as_str()) {
                    continue;
                }
            }
        }
...
pub fn generate_tokens(segments: &[String]) -> Vec<(String, TokenRule)> {
    ...
    for segment in segments {
        ...
        if let Some(ext_os_str) = path_obj.extension() {
            ...
            if ["exe", "bin", "log", "bat", "lnk"].contains(&ext.as_str()) {
                if let Some(name_os_str) = path_obj.file_stem() {
                    ...
                    if tokens_for_segment.contains(&(segment.clone(), TokenRule::Original)) && segment != &current_segment {
        ...
        for (t, _rule) in &tokens_for_segment {
            ...
            if words.len() > 1 {
                let filtered_words: Vec<&str> = words.iter().copied()
...
mod tests {
    ...
    fn test_extract_segments_case1() {
        ...
        assert_eq!(segments, vec!["Heliodor", "流星ワールドアクターGB", "WorldActorGB.exe"]);
    }
